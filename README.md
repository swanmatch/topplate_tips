...
1. Kicad新しいプロジェクトを作っていきなりPcbnewを開いてください。
2. 直下でもディレクトリ掘っても構いませんが、これをダウンロードして配置してください。
3. 読み込んで好きに並べてください。白い枠がキャップのクリアランスなので交差しないようにしてください。
4. Edge.cutsレイヤーを選択して、 線、円弧、円を使って、フレームとネジ穴を作ってください。ネジ穴はM2なら直径2.2mmくらいがいいです。
...
レーザー加工データはInkscapeで作ります。  
Kicadで「ファイル->エクスポート->SVG」でEdge.cutsレイヤーをエクスポートして、Inkscapeに読み込んでください。  
キャンパスのサイズ、線の太さや色などを業者さん指定の方法に従って変更し完了です。簡単ですね。
